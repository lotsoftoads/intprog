...
function App() {
  ...
  // метод по получению данных от API при помощи fetch API
  useEffect(() => {
    fetch("https://api.github.com/users/mui-org/repos")
      .then((res) => res.json())
      .then((res: GithubRepo[]) => {
        res.forEach((repos) => {
    ...
    <Box sx={{ flexGrow: 1 }}>
      ...
      <Container>
        ...
        <Grid container spacing={4} sx={{ pt: "2%" }}>
          {data.map((card) => (
            <Grid item key={card.html_url} xs={12} sm={6} md={4}>
              <Card sx={{ minWidth: 100 }}>
                <CardHeader title={card.name} subheader={card.html_url} />
